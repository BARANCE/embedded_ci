...
下記のパスをPath環境変数を設定する必要があります。注意点として、JenkinsはSYSTEMユーザーで動作するので、ユーザー環境変数は参照されません。環境変数は全てシステム環境変数として設定する必要があります。
...
2. Jenkins上にジョブを作成します。
    ...
    4. ジョブの種類は下記のように決定します。
        - ビルド・転送・テストの各ジョブの場合は、「フリースタイル・プロジェクトのビルド」を選択します。
        - 一括実行ジョブの場合は「パイプライン」を選択します。
    5. 画面下部の「OK」をクリックします。
